...
#define __TCP_CONNECTION_QUEUE_SIZE 1024

namespace tcp {

//!
//! tcp::tcp_server is the class providing TCP Server features.
...
public:
  //!
  //! \return the tcp::tcp_socket associated to the server. (non-const version)
  //!
  tcp_socket& get_socket(void);

  //!
  //! \return the tcp::tcp_socket associated to the server. (const version)
  //!
...
public:
  ...
  //!
  const std::shared_ptr<tcp::io_service>& get_io_service(void) const;

public:
  //!
  //! \return the list of tcp::tcp_client connected to the server.
  //!
  const std::list<std::shared_ptr<tcp::tcp_client>>& get_clients(void) const;
...
private:
  ...
  //! clients
  //!
  std::list<std::shared_ptr<tcp::tcp_client>> m_clients;
