...
, m_on_new_connection_callback(nullptr) { __TCP_LOG(debug, "create tcp_server"); }

tcp_server::~tcp_server(void) {
  __TCP_LOG(debug, "destroy tcp_server");
  stop();
}

void
tcp_server::start(const std::string& host, std::uint32_t port, const on_new_connection_callback_t& callback) {
  if (is_running()) { __TCP_THROW(warn, "tcp_server is already running"); }

  m_socket.bind(host, port);
  m_socket.listen(__TCP_CONNECTION_QUEUE_SIZE);
  ...
  __TCP_LOG(info, "tcp_server running");
}
...
tcp_server::stop(bool wait_for_removal, bool recursive_wait_for_removal) {
  ...
  __TCP_LOG(info, "tcp_server stopped");
}
...
tcp_server::on_read_available(fd_t) {
  try {
    __TCP_LOG(info, "tcp_server received new connection");
    ...
    if (!m_on_new_connection_callback || !m_on_new_connection_callback(client)) {
      __TCP_LOG(info, "connection handling delegated to tcp_server");

      client->set_on_disconnection_handler(std::bind(&tcp_server::on_client_disconnected, this, client));
      m_clients.push_back(client);
    }
    else {
      __TCP_LOG(info, "connection handled by tcp_server wrapper");
    }
  }
  catch (const tcp::tcp_error&) {
    __TCP_LOG(warn, "accept operation failure");
    stop();
  }
}
...
tcp_server::on_client_disconnected(const std::shared_ptr<tcp_client>& client) {
  ...
  __TCP_LOG(debug, "handle server's client disconnection");
...
const std::list<std::shared_ptr<tcp::tcp_client>>&
