...
#if _WIN32
#define __TCP_LENGTH(size) static_cast<int>(size) // for Windows, convert buffer size to `int`
#pragma warning(disable : 4996)                       // for Windows, `inet_ntoa` is deprecated as it does not support IPv6
#else
#define __TCP_LENGTH(size) size // for Unix, keep buffer size as `size_t`
#endif                              /* _WIN32 */
...
, m_host("")
, m_port(0)
, m_type(type::UNKNOWN) { __TCP_LOG(debug, "create tcp_socket"); }
...
, m_host(host)
, m_port(port)
, m_type(t) { __TCP_LOG(debug, "create tcp_socket"); }
...
, m_type(socket.m_type) {
  socket.m_fd   = __TCP_INVALID_FD;
  socket.m_type = type::UNKNOWN;

  __TCP_LOG(debug, "moved tcp_socket");
}
...
tcp_socket::recv(std::size_t size_to_read) {
  ...
  ssize_t rd_size = ::recv(m_fd, const_cast<char*>(data.data()), __TCP_LENGTH(size_to_read), 0);

  if (rd_size == SOCKET_ERROR) { __TCP_THROW(error, "recv() failure"); }

  if (rd_size == 0) { __TCP_THROW(warn, "nothing to read, socket has been closed by remote host"); }
...
tcp_socket::send(const std::vector<char>& data, std::size_t size_to_write) {
  ...
  ssize_t wr_size = ::send(m_fd, data.data(), __TCP_LENGTH(size_to_write), 0);

  if (wr_size == SOCKET_ERROR) { __TCP_THROW(error, "send() failure"); }
...
tcp_socket::listen(std::size_t max_connection_queue) {
  ...
  if (::listen(m_fd, __TCP_LENGTH(max_connection_queue)) == SOCKET_ERROR) { __TCP_THROW(debug, "listen() failure"); }
}
...
tcp_socket::accept(void) {
  ...
  if (client_fd == __TCP_INVALID_FD) { __TCP_THROW(error, "accept() failure"); }
...
tcp_socket::check_or_set_type(type t) {
  if (m_type != type::UNKNOWN && m_type != t) { __TCP_THROW(error, "trying to perform invalid operation on socket"); }
